...
public class Domain<TDomain, TSkill> : IField
    ...
    where TSkill : struct
{
    public Domain(SettingType settingType, TDomain domainType, Dictionary<TSkill, int> skills, int? domainLevel = null)
    {
    ...
    public Dictionary<TSkill, int> Skills { get; }
    
    public string Name => Glossary.GetDomainDictionaryValue(DomainType) + DomainLevelPostfix;
    public object? Value => Skills.ToDictionary(pair => Glossary.GetSkillDictionaryValue(pair.Key), pair => pair.Value.ToString());
